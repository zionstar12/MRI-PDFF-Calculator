...
1. **Continuous MRI-PDFF (%)** — the estimated hepatic proton density fat fraction, displayed with a colour-coded steatosis spectrum bar.
...
├── rf_continuous_model.rds        # Trained RF model — continuous MRI-PDFF
├── rf_multicat_model.rds          # Trained RF model — steatosis grade (S0–S3)
├── man/
│   └── figures/
...
- **Modelling framework:** Random Forest via `ranger` (R package), with separate models for continuous MRI-PDFF regression and four-class steatosis grade classification.
- **Reference standard:** MRI-PDFF measured by magnetic resonance imaging.
- **Input validation:** The calculator flags out-of-range values and prevents prediction when required fields are missing.

For full methodological details, see the associated manuscript (below).
...
- **BK Kim** (Correspondence: BEOMKKIM@yuhs.ac) — Yonsei University College of Medicine
